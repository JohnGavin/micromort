...
curl -X POST "http://localhost:8080/v1/analysis/exchange-matrix" \
  ...
  -d '{"activities": ["Skiing (per day)", "Scuba diving (per dive, trained)", "Skydiving (per jump, US)"]}'
```
...
if (!is.null(hazard)) {
  DT::datatable(
    ...
    DT::formatRound("daily_micromorts", 1) |>
    DT::formatSignif("daily_probability", 3)
}
```
...
resp <- request(base_url) |>
  ...
  req_body_json(list(
    activities = c("Skiing (per day)", "Scuba diving (per dive, trained)")
  )) |>
  req_perform() |>
  resp_body_json()
```
