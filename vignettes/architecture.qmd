...
# `label` is a short human name (e.g. "Pipeline diagram") used ONLY to build
# the fallback message when the target hasn't been built. It is NOT the full
# "requires `tar_make()` to render" sentence — that text is assembled here,
# inside the (include=FALSE) setup chunk, so it never appears as visible
# source in a code-folded call site further down the page. Passing the full
# sentence as a call-site argument previously caused pkgdown's deployed-HTML
# error-pattern gate to false-positive on every rendered page, because
# code-fold: true echoes the chunk source verbatim regardless of whether the
# fallback branch actually ran (micromort#126).
emit_mermaid <- function(target_name, label, caption = NULL, alt_text = NULL) {
  ...
  if (!is.null(diagram)) {
    ...
    cat(paste0("*", label, " requires `tar_make()` to render.*\n"))
  }
}
```
