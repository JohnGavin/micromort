...
show_target("vig_whatis_mundane_table",
  caption = "Seven mundane activities ranked by micromorts (mm). Ratio vs coffee = mm / 0.01 mm (cup of coffee). The Note column flags wine as a chronic daily risk (microlives/day, not a per-event acute risk); it is included for narrative continuity at its Wikipedia/Spiegelhalter source value of 0.5 mm/day. All other rows are acute per-event risks from common_risks(). Source: micromort package.")
```
