...
if (!is.null(whatif)) {
  .pull1 <- function(df, act, col) {
    rows <- dplyr::filter(df, activity == act)
    if (nrow(rows) != 1L) {
      cli::cli_abort(c(
        "x" = "Expected exactly one row for {.val {act}} in {.code vig_confounding_demographic_whatif}.",
        "i" = "Got {nrow(rows)} row(s)."
      ))
    }
    dplyr::pull(rows, !!rlang::sym(col))
  }
  anaes_default <- .pull1(whatif, "General anaesthesia (elective)", "default_mm")
  anaes_aged   <- .pull1(whatif, "General anaesthesia (elective)", "aged_mm")
  bath_default <- .pull1(whatif, "Taking a bath (age-conditioned)", "default_mm")
  bath_aged    <- .pull1(whatif, "Taking a bath (age-conditioned)", "aged_mm")
} else {
