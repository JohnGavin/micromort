...
#
# PROVENANCE: quiz_pairs.csv is pre-computed by quiz_pairs() which pulls
# from atomic_risks(). Each row's micromort values trace to source_url in
# atomic_risks(). The help_url_a / help_url_b columns carry per-activity
# source URLs. See data-raw/generate_quiz_csv.R for the generation pipeline.
#
...
// ---- Timer (#58) ----
// Set to Date.now() when the first question renders; null until then.
...
// Currently logs to console only until form fields are created.
...
function submitScore(score, total, difficulty, nQuestions, nSkipped, meanConf) {
  ...
  console.log('Quiz metadata (acute):', meta);
  var btn = document.getElementById('submit_btn');
  if (btn) {
    btn.disabled = true;
...
function renderQR(containerId, size) {
  ...
  el.innerHTML = '<div style=\"display:flex;align-items:center;gap:16px;margin-top:12px;padding:8px 0;\">' +
    '<span style=\"display:inline-block;max-width:110px;max-height:110px;\">' + qr.createImgTag(size || 3) + '</span>' +
...
setTimeout(function() { renderQR('qr-results', 3); }, 500);
  ...
  tagList(
    ...
    div(class = "text-center mb-3",
      tags$button(id = "feedback_submit_btn", class = "btn btn-outline-secondary btn-sm",
        onclick = "submitFeedback('acute')", "Submit Feedback")
    ),
    tags$hr(),
    # ---- Streak + navigation ----
    ...
    if (!isTRUE(state$attempt_logged)) {
      ...
      tags$script(HTML(sprintf(
        paste0("(function(){",
               "recordScoreHistory('acute', %s, %d, %d, [%s], [%s]);",
               "var keys=[%s];",
               "var corr=[%s];",
        ...
        paste(sprintf("simpleHash('%s|%s')",
                      gsub("'", "\\\\'", pairs$activity_a[seq_len(n)]),
                      gsub("'", "\\\\'", pairs$activity_b[seq_len(n)])),
              collapse = ","),
...
server <- function(input, output, session) {
  ...
  # risk-ranking-quiz.qmd / microlife-quiz.qmd fix, PRs #172/#173/#174):
  # the answer is recorded on the choice click, but revealed[] only flips
  # to TRUE once confidence is given -- otherwise the confidence prompt
  # would appear simultaneously with the right/wrong reveal, invalidating
...
"General anaesthesia (elective)","Ecstasy/MDMA (per dose)",13,"Drugs",0,"per dose",2,"Medical",0,"per event",6.5,"easy","a","Risk from hyperthermia, serotonin syndrome, and contaminated supply.","https://en.wikipedia.org/wiki/MDMA","Elective anaesthesia mortality varies ~100-fold by age: ~0.5 mm for under-60s to ~50 mm for 80+ (Lancet). Emergency anaesthesia is ~10 mm across all ages.","https://doi.org/10.1016/S0140-6736(02)11626-6"
...
"Airline pilot (annual radiation)","Walking (1 trip, NYC)",0.09,"Travel",0,"per trip",0.15,"Occupation",100,"per year",1.66666666666667,"hard","b","Average pedestrian fatality risk per walking trip in New York City (~110 deaths/year, 41% of trips on foot). Outer boroughs have higher per-pedestrian risk than Times Square.","https://www.nature.com/articles/s44284-025-00383-y","Cumulative cosmic radiation exposure from ~700 flight hours per year at altitude.","https://en.wikipedia.org/wiki/Airline_pilot"
"All US workers baseline (per work day)","Walking (1 trip, NYC)",0.09,"Travel",0,"per trip",0.15,"Occupation",0,"per day",1.66666666666667,"hard","b","Average pedestrian fatality risk per walking trip in New York City (~110 deaths/year, 41% of trips on foot). Outer boroughs have higher per-pedestrian risk than Times Square.","https://www.nature.com/articles/s44284-025-00383-y","Average occupational fatality rate across all US industries; baseline for comparison.","https://www.bls.gov/iif/fatal-injuries-tables.htm"
...
"Barium enema (radiation per scan)","Scuba diving, trained (per dive)",5,"Sport",0,"per dive",3,"Medical",0,"per event",1.66666666666667,"hard","a","Single-dive risk for trained divers from decompression sickness and equipment failure.","https://en.wikipedia.org/wiki/Scuba_diving","Moderate radiation dose from fluoroscopic imaging of the large intestine.","https://en.wikipedia.org/wiki/Barium_enema"
...
"Bed fall (per night)","Annual tourist flyer (annual cosmic)",0.006,"Travel",0,"per year",0.004,"Daily Life",0,"per night",1.5,"hard","a","Minimal cosmic radiation from occasional holiday flights (~6,000 miles/year).","https://en.wikipedia.org/wiki/Cosmic_ray","Falling out of bed kills ~450 Americans/year. Risk is massively age-dependent: 0.004 mm/night under 65 vs 10.2 mm/night for 85+ males (2,500-fold difference).","https://www.cdc.gov/nchs/products/databriefs/db532.htm"
...
"General anaesthesia (elective)","Rock climbing (per day)",3,"Sport",0,"per day",2,"Medical",0,"per event",1.5,"hard","a","Risk from falls, rockfall, and equipment failure on natural rock. Per full day of climbing.","https://en.wikipedia.org/wiki/Rock_climbing","Elective anaesthesia mortality varies ~100-fold by age: ~0.5 mm for under-60s to ~50 mm for 80+ (Lancet). Emergency anaesthesia is ~10 mm across all ages.","https://doi.org/10.1016/S0140-6736(02)11626-6"
"General anaesthesia (elective)","COVID-19 bivalent booster (age 65-79)",3,"COVID-19",0,"11 weeks (2022)",2,"Medical",0,"per event",1.5,"hard","a","Bivalent-boosted 65-79 year-olds had lowest but still measurable COVID-19 risk.","https://www.cdc.gov/mmwr/volumes/72/wr/mm7206a3.htm","Elective anaesthesia mortality varies ~100-fold by age: ~0.5 mm for under-60s to ~50 mm for 80+ (Lancet). Emergency anaesthesia is ~10 mm across all ages.","https://doi.org/10.1016/S0140-6736(02)11626-6"
"General anaesthesia (elective)","Commercial fishing (per work day)",3,"Occupation",0,"per day",2,"Medical",0,"per event",1.5,"hard","a","Extreme weather, vessel sinking, and deck machinery make fishing one of the deadliest occupations.","https://www.bls.gov/iif/fatal-injuries-tables.htm","Elective anaesthesia mortality varies ~100-fold by age: ~0.5 mm for under-60s to ~50 mm for 80+ (Lancet). Emergency anaesthesia is ~10 mm across all ages.","https://doi.org/10.1016/S0140-6736(02)11626-6"
...
"Shark encounter (ocean swim)","Walking (1 trip, NYC)",0.09,"Travel",0,"per trip",0.06,"Wildlife",100,"per swim",1.5,"hard","a","Average pedestrian fatality risk per walking trip in New York City (~110 deaths/year, 41% of trips on foot). Outer boroughs have higher per-pedestrian risk than Times Square.","https://www.nature.com/articles/s44284-025-00383-y","Shark attack fatality risk per ocean swim, based on ISAF incident data.","https://www.floridamuseum.ufl.edu/shark-attacks/"
