...
#
# WHY INLINE: This quiz runs in Shinylive (WebR/WASM in the browser).
# WebR cannot load the micromort package because arrow is not available
# in WASM. So quiz data MUST be embedded as inline CSV.
#
# PIPELINE TRACKED: The targets pipeline verifies this embedded CSV
# matches the canonical output of quiz_pairs() via the
# vig_quiz_csv_check target. If data drifts, that target returns "STALE".
#
# TO REGENERATE: Run `Rscript data-raw/generate_quiz_csv.R` then paste
# the output CSV here. The canonical copy is also saved to
# inst/extdata/vignettes/quiz_pairs.csv for traceability.
