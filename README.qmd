...
See the [Introduction vignette](https://johngavin.github.io/micromort/articles/introduction.html) for detailed examples.
...
```{r plot, fig.cap="Risk comparison in micromorts (log scale). Bars show death probability per event. COVID-19 and other risks shown in separate panels for clarity."}
...
Endpoints:
- `GET /v1/acute` - Acute risks dataset (micromorts per event)
- `GET /v1/chronic` - Chronic risks dataset (microlives per day)
- `GET /v1/sources` - Source registry
- `GET /v1/hazard?age=35` - Daily hazard rate (micromorts per day)
