...
plan_site_build <- list(
  ...
  # Render portfolio builder (Shinylive article)
  #
  # pkgdown's build_quarto_articles() refuses to overwrite
  # docs/articles/portfolio_shinylive.html because that file was not
  # originally produced by pkgdown's own quarto-article wrapper (it lacks
  # pkgdown's "generated by pkgdown" signature) — pkgdown logs
  # "Skipping 'articles/portfolio_shinylive.html': not generated by
  # pkgdown" and silently leaves the existing (stale) file in place. This
  # mirrors site_quiz_shinylive/site_chronic_shinylive/site_ranking_shinylive:
  # render directly with quarto (writing into vignettes/, per
  # vignettes/_quarto.yml's `output-dir: .`) and let site_deploy_shinylive
  # copy the fresh output into docs/articles/.
  targets::tar_target(
    site_portfolio_shinylive,
    {
      force(site_pkgdown)

      qmd_path <- "vignettes/portfolio_shinylive.qmd"
      if (!fs::file_exists(qmd_path)) {
        cli::cli_alert_info("Portfolio shinylive qmd not found — skipping")
        return(list(qmd = qmd_path, skipped = TRUE, timestamp = Sys.time()))
      }
      ...
      cli::cli_alert_success("Portfolio shinylive rendered")
      list(qmd = qmd_path, output = result, timestamp = Sys.time())
    }
  ),
