...
#'
#' Returns daily micromorts from infectious diseases for one or more countries,
...
#' @seealso [chronic_disease_risks()] for chronic disease micromort risks by
#'   country.
#' @examples
...
infectious_disease_risks <- function(country = "GBR", year = NULL) {
  ...
  csv_path <- system.file(
    "extdata", "owid_infectious_deaths.csv",
    package = "micromort",
    mustWork = TRUE
  )
  ...
  raw <- readr::read_csv(
    ...
    col_types = readr::cols(
      country_iso3    = readr::col_character(),
    ...
    dplyr::rename(iso3 = "country_iso3", country = "country_name")
