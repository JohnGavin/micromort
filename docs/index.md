...
authoritative sources including Wikipedia, CDC MMWR, and academic
literature. This is a reference dataset for risk comparison and
communication — not a time-series analysis tool or epidemiological
modelling framework.
...
Package architecture diagram (click to expand; renders on GitHub)
...
for detailed package diagrams.

See the [Architecture
vignette](https://johngavin.github.io/micromort/articles/architecture.html)
for detailed diagrams of the data pipeline, function hierarchy, and user
journey.
...
Use `lle(prob, life_expectancy = ...)` and
[`as_microlife()`](https://johngavin.github.io/micromort/reference/as_microlife.md)
to convert at any age. See the [Age-Based Hazard
...
Generate geography-specific quiz pairs comparing disease risk across
countries:
...
| LLE | Loss of Life Expectancy | Expected lifespan reduction from a risk, in minutes. See [`lle()`](https://johngavin.github.io/micromort/reference/lle.md). |
...
| VSL | Value of Statistical Life | Monetary value society places on preventing one death (~\$10M USD). See [`value_of_micromort()`](https://johngavin.github.io/micromort/reference/value_of_micromort.md). |
...
See [LICENSE](https://johngavin.github.io/micromort/LICENSE) for
details.
