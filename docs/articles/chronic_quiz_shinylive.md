...
question_ui <- function(state) {
  ...
  # Awaiting-confidence: an answer has been picked but not yet revealed
  # (reveal is now gated on confidence -- see conf_* handlers above).
  awaiting_confidence <- !is.na(answer) && !revealed
  ...
  if (awaiting_confidence) {
    left_class <- paste(left_class, if (chose_left) "quiz-btn-neutral" else "")
    right_class <- paste(right_class, if (chose_right) "quiz-btn-neutral" else "")
  } else if (revealed) {
    ...
    confidence_ui <- div(class = "text-center mt-2",
      ...
      div(class = "option-btn-group justify-content-center", id = "grp_conf",
        lapply(conf_choices, function(v) {
          actionButton(paste0("conf_", v), paste0(v, "%"), class = "option-btn")
        })
      )
    )
  } else if (revealed && !is.na(state$confidences[q])) {
    confidence_ui <- div(class = "text-center mt-2",
      tags$small(class = "text-muted", sprintf("Your confidence: %d%%", as.integer(state$confidences[q])))
    )
  }
  ...
  correct_so_far <- sum(vapply(all_qs, function(i) {
    state$revealed[i] && !is.na(state$answers[i]) && state$answers[i] == state$pairs$answer[i]
  }, logical(1)))
      ...
      div(class = "col-5",
        if (is.na(answer)) {
          ...
          div(class = left_class,
              style = if (awaiting_confidence && !chose_left) "opacity:0.5" else NULL,
      ...
      div(class = "col-5",
        if (is.na(answer)) {
          ...
          div(class = right_class,
              style = if (awaiting_confidence && !chose_right) "opacity:0.5" else NULL,
...
server <- function(input, output, session) {
  ...
  # Confidence capture now GATES the reveal (matches micromort-quiz.qmd /
  # risk-ranking-quiz.qmd / microlife-quiz.qmd / quiz_shinylive.qmd fix,
  # PRs #172/#173/#174/#175): the answer is recorded on the choice click,
  # but revealed[] only flips to TRUE once confidence is given -- otherwise
  # the confidence prompt would appear simultaneously with the right/wrong
  # reveal, invalidating the calibration measurement.
  observeEvent(input$conf_0,   { state$confidences[state$current_q] <- 0;   state$revealed[state$current_q] <- TRUE })
  observeEvent(input$conf_25,  { state$confidences[state$current_q] <- 25;  state$revealed[state$current_q] <- TRUE })
  observeEvent(input$conf_50,  { state$confidences[state$current_q] <- 50;  state$revealed[state$current_q] <- TRUE })
  observeEvent(input$conf_75,  { state$confidences[state$current_q] <- 75;  state$revealed[state$current_q] <- TRUE })
  observeEvent(input$conf_100, { state$confidences[state$current_q] <- 100; state$revealed[state$current_q] <- TRUE })
