...
`.claude/context/` or `inst/ctx/` — those don’t exist. ALWAYS use
`ctx_audit()` from `plan_pkgctx.R`.
...
- PHI/patient data: NEVER commit without anonymization (see
  `medical-data-anonymization` global rule)
- DuckDB: use `connect_duckdb_secure()` pattern (see `duckdb-security`
  global rule)
...
- suppressWarnings(as.\*) forbidden — see
  `suppress-warnings-antipattern` rule
